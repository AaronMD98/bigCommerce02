  ...
  return (
    <ComponentsProductCard key={product.entityId}>
      <ProductCardImage>
        ...
        >
          {product.defaultImage && product.path ? (
            <Link className="flex" href={product.path}>
              <BcImage
                alt={product.defaultImage.altText ?? product.name ?? ''}
                className="z-10 h-full rounded-md border object-cover transition-opacity duration-300 ease-in-out hover:opacity-0"
                fill
              ...
              {product.images.edges[2] && (
                <BcImage
                  alt={product.images.edges[1]?.node.altText ?? product.name ?? ''}
                  className="z-10 rounded-md object-cover opacity-0 transition-opacity duration-300 ease-in-out hover:opacity-100"
                  fill
                  priority={imagePriority}
                  sizes="(max-width: 768px) 50vw, (max-width: 1536px) 25vw, 500px"
